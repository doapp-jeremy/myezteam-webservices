...
public abstract class TeamController {
  ...
  private final LoadingCache<Long, List<Team>> ownerTeams = CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.MILLISECONDS)
  ...
  private final LoadingCache<Long, List<Team>> managerTeams = CacheBuilder.newBuilder()
  ...
  private final LoadingCache<Long, List<Team>> playerTeams = CacheBuilder.newBuilder()
      .expireAfterAccess(1, TimeUnit.MILLISECONDS)
      .maximumSize(1000).build(new CacheLoader<Long, List<Team>>() {
        @Override
        public List<Team> load(Long userId) throws Exception {
